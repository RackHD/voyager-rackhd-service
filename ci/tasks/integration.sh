...
scp -i ssh.key -o "StrictHostKeyChecking no" -r voyager-rackhd-service  ${INTEGRATION_VM_USER}@${INTEGRATION_VM_IP}:$project_path

integration_file=$PWD/voyager-rackhd-service/ci/tasks/run-integration.sh
